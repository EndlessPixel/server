...
const LicenseHeaderCard = ({ info }: { info: LicenseInfo }) => (
  <Card className="border-0 bg-linear-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20">
    ...
    <CardContent className="flex items-center gap-3 flex-wrap">
      ...
      <Button size="sm" variant="ghost" onClick={() => window.open(info.officialUrl, "_blank")}><ExternalLink className="w-4 h-4 mr-2" />官方文本</Button>
      <Button
        size="sm"
        variant="ghost"
      ...
      <CardContent>
        <div className="flex flex-wrap gap-2">
          {category.items.map((item) => (
            <Badge key={item} className={colors.badge}>{item}</Badge>
          ))}
        </div>
      </CardContent>
    </Card>
