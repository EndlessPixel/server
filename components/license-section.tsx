...
const LicenseHeaderCard = ({ info }: { info: LicenseInfo }) => (
  <Card className="border-0 bg-linear-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20">
    <CardHeader>
      <div className="flex items-center gap-4 p-3 rounded-2xl bg-linear-to-r from-slate-800 to-slate-900 text-white shadow-lg">
      ...
      <CardContent>
        <div className="flex flex-wrap gap-2">
          {category.items.map((item) => (
            <Badge key={item} className={colors.badge}>
              {item}
            </Badge>
          ))}
        </div>
