...
export function TeamSection() {
  ...
  const getDepartmentIcon = (dept: string) => {
    const icons: { [key: string]: any } = { "团队创始人": Crown, "服务器创始人": Server, "服主": Crown, "服务器": Server, "代码部": Sparkles, "维护部": Shield, "网络部": Globe, "宣传部": Sparkles, "创始人": Crown, "摸鱼部": Coffee, "域名": Globe, "杂务部": Wrench, }
    return icons[dept] || Users
  }
  return (
    <Card className="border-l-4 border-blue-500 dark:border-blue-600 bg-card/95 dark:bg-card/95">
      <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2"><div className="inline-flex items-center justify-center w-9 h-9 bg-blue-50 dark:bg-blue-950/40 rounded-lg"><Users className="w-5 h-5 text-blue-600 dark:text-blue-400" /></div>团队介绍</CardTitle></CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {teamMembers.map((member) => (
            <div key={member.name} className={` rounded-lg p-6 transition-all duration-300 hover:shadow-md hover:scale-102 ${member.bgColor} ${member.borderColor} group`}>
              <h3 className="text-lg font-semibold mb-2 group-hover:text-foreground/80 transition-colors">{member.name}</h3>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {member.departments.map((dept) => {
                    ...
                    return (
                      <Badge key={dept} variant="outline" className="text-xs bg-background/50 dark:bg-background/80 flex items-center gap-1 px-2 py-1"><DeptIcon className="w-3 h-3" />{dept}</Badge>
                    );
                  })}
                </div>
              </div>
